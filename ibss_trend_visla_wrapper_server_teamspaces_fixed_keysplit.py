...
VISLA_API_KEY = os.getenv("VISLA_API_KEY", "").strip()
VISLA_API_SECRET = os.getenv("VISLA_API_SECRET", "").strip()
...
def _split_credential(raw: str) -> tuple[str, str]:
    if VISLA_API_KEY and VISLA_API_SECRET:
        return VISLA_API_KEY, VISLA_API_SECRET
    if "." not in raw:
        raise ValueError("Set VISLA_API_KEY and VISLA_API_SECRET, or use VISLA_CREDENTIAL in `api_key.api_secret` format.")
    # Visla API keys themselves may contain a dot, so split from the right.
    return raw.rsplit(".", 1)
...
def _create_video_core(req: VideoRequest) -> dict:
    ...
    if not (VISLA_CREDENTIAL or (VISLA_API_KEY and VISLA_API_SECRET)):
        return _default_fail({}, "manual_fields", "", [], diagnostics, summary_markdown="缺少 VISLA_API_KEY / VISLA_API_SECRET（或 VISLA_CREDENTIAL）环境变量。", error_message="Visla credentials are not configured on Render.")
...
@app.get("/visla/teamspaces")
def visla_teamspaces() -> Dict[str, Any]:
    diagnostics: List[str] = ["Started teamspace lookup"]
    if not (VISLA_CREDENTIAL or (VISLA_API_KEY and VISLA_API_SECRET)):
        return {
            "status": "failed",
            "error_message": "Visla credentials are not configured.",
            "teamspace_uuids": [],
            "diagnostics_text": "\n".join(diagnostics),
        }
    try:
    ...
    rsp, raw, status_code = _visla_request("GET", "/workspace/list-my-teamspace", api_key, api_secret)
    diagnostics.append(f"HTTP status={status_code}")
    ...
    while queue:
        ...
        if isinstance(item, dict):
            for k, v in item.items():
                if k in {"teamspaceUuid", "teamSpaceUuid", "teamspace_uuid", "uuid"} and isinstance(v, str) and len(v) >= 8:
                    if v not in uuids:
                        uuids.append(v)
                if isinstance(v, (dict, list)):
                    queue.append(v)
    ...
    return {
        "status": "ok" if status_code == 200 and uuids else "failed",
        "teamspace_uuids": uuids,
        "suggested_teamspace_uuid": uuids[0] if uuids else "",
        "raw_response": rsp,
        "raw_text": raw[:3000],
        "diagnostics_text": "\n".join(diagnostics),
    }
